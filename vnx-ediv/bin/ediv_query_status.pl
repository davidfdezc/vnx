...
sub getDBConfiguration {
	...
	###########################################################
	# Subroutine to obtain information of simulations
	###########################################################
sub simulationInfo{
	my $dbh = DBI->connect($db_connection_info,$db_user,$db_pass);
	
	if ($simulation_name eq undef){
			my $query_string = "SELECT `host`,`simulation` FROM hosts WHERE status='running'";
			my $query = $dbh->prepare($query_string);
			$query->execute();
			
			while (@simulations = $query->fetchrow_array()) {
				print ("The simulation $simulations[1] is running at host $simulations[0]\n");
			}
			
			$query->finish();
			
		} else{
			my $query_string = "SELECT `name`,`host`,`simulation` FROM vms WHERE simulation='$simulation_name'";
			my $query = $dbh->prepare($query_string);
			$query->execute();
			
			while (@simulations = $query->fetchrow_array()) {
				print ("The simulation $simulations[2] has the virtual machine $simulations[0] that is in host $simulations[1]\n");
			}
